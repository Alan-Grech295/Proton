...
namespace Proton
{
	Ref<Model> ModelSerializer::DeserializeModel(const std::filesystem::path& modelPath, UUID uuid)
	{
		Assimp::Importer imp;
		...
		Ref<Model> model = CreateRef<Model>(uuid);
		...
		for (int i = 0; i < pScene->mNumMaterials; i++)
		{
			...
			Ref<Material> mat = DeserializeAssimpMaterial(basePath, *pScene->mMaterials[i], materialUUID);
			materials[i] = mat;
		...
		{
			DeserializeMesh(&model->m_Meshes[i], *pScene->mMeshes[i], modelPath.string(), materials);
		}
		...
		// Counting the nodes
		int count = 0;
		nodes.emplace(pScene->mRootNode);
		while (!nodes.empty())
		{
	...
	Ref<Material> ModelSerializer::DeserializeAssimpMaterial(const std::string& basePath, const aiMaterial& aiMat, UUID uuid)
	{
		namespace dx = DirectX;

		// Single pass
		Ref<Material> material = CreateRef<Material>(uuid);
		...
		if (aiMat.GetTexture(aiTextureType_DIFFUSE, 0, &texFileName) == aiReturn_SUCCESS)
		{
			...
			Ref<Texture2D> diffuse = Texture2D::Create(basePath + texFileName.C_Str());
		...
		if (aiMat.GetTexture(aiTextureType_NORMALS, 0, &texFileName) == aiReturn_SUCCESS)
		{
			...
			Ref<Texture2D> normal = Texture2D::Create(basePath + texFileName.C_Str(), 2);
