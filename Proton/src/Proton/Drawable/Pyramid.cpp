...
#include "Proton/Geometry/Cone.h"

namespace Proton
{
	Pyramid::Pyramid(WindowsGraphics& gfx,
		std::mt19937& rng,
		...
		phi(adist(rng))
	{
		...
		if (!IsStaticInitialised())
		{
			...
			auto pvs = std::make_unique<VertexShader>(gfx, gfx.GetShaderPath("ColorBlendVS.cso"));
			auto pvsbc = pvs->GetBytecode();
			AddStaticBind(std::move(pvs));

			AddStaticBind(std::make_unique<PixelShader>(gfx, gfx.GetShaderPath("ColorBlendPS.cso")));
